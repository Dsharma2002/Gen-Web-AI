...
import { getSandbox, lastAssisstantTextMessageContent } from "./utils";
import { z } from "zod";
    ...
    const codeAgent = createAgent<AgentState>({
      ...
      lifecycle: {
        onResponse: async ({ result, network }) => {
          const lastAssisstantMessageText =
            lastAssisstantTextMessageContent(result);

          if (lastAssisstantMessageText && network) {
            if (lastAssisstantMessageText.includes("<task_summary>")) {
              network.state.data.summary = lastAssisstantMessageText;
            }
          }
          return result;
        },
